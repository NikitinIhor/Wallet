const walletClosed=document.querySelector(".wallet-closed"),walletOpened=document.querySelector(".wallet-opened"),walletBox=document.querySelector(".wallet-box"),walletCloseBtn=document.querySelector(".wallet-close-btn"),moneyBody=document.querySelector(".money-body"),myCash=document.querySelector(".my-cash"),income=document.querySelector(".income"),expense=document.querySelector(".expense"),addTransaction=document.querySelector(".add"),clearAllTransactions=document.querySelector(".delete"),popupSave=document.querySelector(".popup-save"),popupClose=document.querySelector(".popup-close"),popup=document.querySelector(".popup"),popupTitle=document.querySelector("#title"),popupSum=document.querySelector("#sum"),popupCategory=document.querySelector("#category"),errorTitle=document.querySelector(".error-title"),errorSum=document.querySelector(".error-sum"),errorCategory=document.querySelector(".error-category"),deleteTransaction=document.querySelector(".transaction-delete");let myInterval,ID=0,transactionIcon,selectedIcon,moneyArr=[0];walletClosed.addEventListener("click",()=>{walletClosed.classList.add("close"),walletOpened.classList.add("open"),setTimeout(walletMove,1e3),setTimeout(openWalletBox,3300),myInterval=setInterval(moneyFall,100)});const walletMove=()=>{walletOpened.classList.add("move-left")},openWalletBox=()=>{walletBox.style.display="flex",walletCloseBtn.style.display="block",clearInterval(myInterval)},moneyFall=(walletCloseBtn.addEventListener("click",()=>{walletCloseBtn.style.display="none",walletBox.style.display="none",walletOpened.classList.remove("move-left"),walletOpened.classList.remove("open"),walletClosed.classList.remove("close")}),()=>{var e=document.createElement("money-item");e.classList.add("money"),e.textContent="💵",document.body.append(e),e.style.left=Math.random()*window.innerWidth+"px",e.style.animationDuration=5*Math.random()+3+"s"}),checkForm=(addTransaction.addEventListener("click",()=>{popup.style.display="grid"}),popupClose.addEventListener("click",()=>{popup.style.display="none"}),()=>{var e=document.createElement("div");e.classList.add("transaction"),e.setAttribute("id",ID),newTransactionIcon(selectedIcon),e.innerHTML=`
